...
import { getWritableCatalogScope } from '../permissions/catalogPermissions.js';
...
export async function getTrashOrArchiveItems(
      ...
      ? await Promise.all([
          prisma.process.findMany({
            ...
            select: {
              ...
              name: true,
              contextId: true,
              ...(isTrash ? { deletedAt: true } : { archivedAt: true }),
            },
          }),
  ...
  for (const p of procs) {
    if (scopeLead || writableCtxSet.has(p.contextId)) {
      items.push({
        ...
        contextName: '-',
        ...(isTrash
          ? { deletedAt: (p as { deletedAt: Date | null }).deletedAt?.toISOString() ?? '' }
          : { archivedAt: (p as { archivedAt: Date | null }).archivedAt?.toISOString() ?? '' }),
      });
    }
  }
  ...
  for (const d of docs) {
    items.push({
      ...
      ...(isTrash
        ? { deletedAt: (d as { deletedAt: Date | null }).deletedAt?.toISOString() ?? '' }
        : { archivedAt: (d as { archivedAt: Date | null }).archivedAt?.toISOString() ?? '' }),
    });
  }
  return { items };
}
